...
@app.task
async def initiate_pipeline_for_courses():
    await asyncio.sleep(1)  # Optional: allow time for app startup
    ...
    for course in course_list:
        ...
        plot_prompt = (
            ...
            f"tailored for {level} learners. The outline should include clear learning objectives, "
            f"module titles, and a brief description for each."
        )

        with trace(f"Curriculum Planning Phase for {title}"):
            result = await Runner.run(plot_agent, plot_prompt)
